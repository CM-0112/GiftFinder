...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    ...
    { href: "/wishlist", label: "My Wishlist", short: "Wishlist" },
    { href: "/connections", label: "Connections", short: "Connections" },
    { href: "/gifts-giving", label: "Items I've Claimed", short: "Claimed" },
  ];
            ...
            {menuOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#FFFFFF", border: "1px solid #E5E0D8", borderRadius: "12px", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", minWidth: "180px", overflow: "hidden", zIndex: 100 }}>
                ...
                <button onClick={() => { setMenuOpen(false); signOut({ callbackUrl: "/login" }); }}
                  style={{ display: "block", width: "100%", textAlign: "left", padding: "0.65rem 1rem", fontFamily: "'DM Sans', sans-serif", fontSize: "0.875rem", color: "#DC2626", background: "none", border: "none", cursor: "pointer" }}>
                  Sign out
                </button>
              </div>
            )}
            ...
            {navLinks.map(link => {
              ...
              return (
                <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
                  style={{ display: "block", padding: "0.9rem 1.25rem", fontFamily: "'DM Sans', sans-serif", fontSize: "0.95rem", fontWeight: active ? 500 : 400, color: active ? "#E8622A" : "#57534E", textDecoration: "none", borderBottom: "1px solid #F5F0EB", background: active ? "#FFF0E8" : "transparent" }}>
                  {link.label}
                </Link>
              );
            })}
